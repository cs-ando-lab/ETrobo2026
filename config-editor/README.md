...
- ETラリーのゲートは、L/Rコースと色を選んでフィールドをクリックして配置できます。
- LコースとRコースのゲート座標は独立して保持・保存します。
- 赤・黄ゲートは横向き、青ゲートは縦向きに固定されます。
- ゲート配置ゾーンの灰色点は、左上を`X=1, Y=1`、右下を`X=5, Y=5`として扱います。
- 範囲・隣接・向き・重複の検証は、L/Rそれぞれのコース内で行います。

ゲート配置UIの`Lコース`／`Rコース`ボタンで編集対象を切り替えます。Lコースでは
`ETRALLY_L_*_GATE_*`、Rコースでは`ETRALLY_R_*_GATE_*`の定数を更新します。
一方のコースで配置を変更しても、もう一方のコースの座標には影響しません。

Rコースの配置画面はLコースを左右の鏡写しにした座標系で表示します。

| 画面 | 左端のX | 右端のX | 上端のY | 下端のY |
| --- | ---: | ---: | ---: | ---: |
| Lコース | 1 | 5 | 1 | 5 |
| Rコース | 5 | 1 | 1 | 5 |

Rコースでは背景画像も左右反転し、クリックした画面上の位置を反転後のX座標へ変換して
`Config.h`へ保存します。Y座標は反転しません。これはConfig Editor上の表示・入力変換であり、
`RallyRoute`内の座標系には変換処理を追加しません。
